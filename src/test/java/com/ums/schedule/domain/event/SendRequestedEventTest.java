package com.ums.schedule.domain.event;

import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

class SendRequestedEventTest {
    @Test
    @DisplayName("SendRequestStatus가 CREATE이면, 익셉션이 발생한다.")
    void shouldThrowException_whenSendRequestStatusIsCreate() {

    }

    @Test
    @DisplayName("SendRequestStatus가 READY이면, SendReuqestEvent의 EventType은 REQUEST가 반환된다.")
    void shouldReturnSendRequestEventTypeIsRequest_whenSendRequestStatusIsReady() {

    }

    @Test
    @DisplayName("SendRequestStatus가 READY이면, REQUEST가 반환된다.")
    void shouldReturnSendRequestStatusIsRequest_whenSendRequestStatusIsReady() {

    }

    @Test
    @DisplayName("SendRequestStatus가 REQUEST이면, 익셉션이 발생한다.")
    void shouldThrowException_whenSendRequestStatusIsRequest() {

    }

    @Test
    @DisplayName("SendRequestStatus가 HOLIDNG이면, 익셉션이 발생한다.")
    void shouldThrowException_whenSendRequestStatusIsHolding() {

    }

    @Test
    @DisplayName("SendRequestStatus가 SENDING이면 익셉션이 발생한다.")
    void shouldThrowException_whenSendRequestStatusIsSending() {

    }

    @Test
    @DisplayName("SendRequestStatus가 COMPLETE이면 익셉션이 발생한다.")
    void shouldThrowException_whenSendRequestStatusIsComplete() {

    }
}